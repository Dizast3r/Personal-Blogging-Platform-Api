...
public class BlogServiceImplTest {
    ...
    public void createBlog_WithNewTags_ShouldSaveAndAssignTags() {
        ...
        when(tagRepository.findByNombre("java")).thenReturn(Collections.emptyList());
        
        // Mock: Saving the tag (repo returns the saved entity)
        // Note: Logic trims/lowers name BEFORE saving
        ...
        Tag resultTag = result.getBlogTags().iterator().next();
        
        // Check normalization
        assertEquals("java", resultTag.getNombre());
        assertNotNull(resultTag.getTagId()); // Should be the saved one
    ...
    public void createBlog_WithExistingTags_ShouldReuseTagFromDB() {
        ...
        assertEquals(existingDbTag.getTagId(), resultTag.getTagId());
    ...
    public void createBlog_WithMixedTags_ShouldHandleBothcorrectly() {
        ...
        // Mocking
        // "python" -> New (Empty list)
        when(tagRepository.findByNombre("python")).thenReturn(Collections.emptyList());
        // "java" -> Exists
        ...
        // Check IDs
        boolean foundJava = result.getBlogTags().stream().anyMatch(t -> t.getTagId().equals(existingJava.getTagId()));
        boolean foundPython = result.getBlogTags().stream().anyMatch(t -> t.getTagId().equals(savedPython.getTagId()));
